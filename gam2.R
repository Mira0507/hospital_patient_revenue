...
da5_RMSE_SD <- rbind(da_gam5_RMSE, da_gam4_SD) %>%
...
pseudoR2 <- data.frame(Model1 = summary(gam_mod1)$dev.expl,
                       Model2 = summary(gam_mod2)$dev.expl,
                       Model3 = summary(gam_mod3)$dev.expl)
...
gain_curve_In <- gain_curve(da_gam4, "In_Pred", "")
gain_curve_Out <- gain_curve(da_gam4, "Out_Pred", "")
gain_curve_Combo <- gain_curve(da_gam4, "Combo_Pred", "")
